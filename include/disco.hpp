...
template<typename T> std::vector<std::pair<T, float>> similar(const Map<T>& map, const DenseMatrix& factors, const std::vector<float>& norms, const T& id, size_t count) {
    ...
    std::ranges::partial_sort(predictions.begin(), predictions.begin() + std::min(predictions.size(), count), predictions.end(), [](const std::pair<size_t, float>& a, const std::pair<size_t, float>& b) {
